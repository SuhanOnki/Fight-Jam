...
namespace NINESOFT.TUTORIAL_SYSTEM
{
    public class TutorialManager : MonoBehaviour
    {
        ...
        [Space(10)] public List<Tutorial> Tutorials = new List<Tutorial>();
        ...
        {
            get { return PlayerPrefs.GetInt("ns_tutorialManagerCompleted_" + SaveKey) == 1; }
            private set { PlayerPrefs.SetInt("ns_tutorialManagerCompleted_" + SaveKey, value ? 1 : 0); }
        }
        ...
        private void OnValidate()
        {
            try
            {
                if (gameObject.activeInHierarchy)
                    PrefabUtility.UnpackPrefabInstance(gameObject, UnityEditor.PrefabUnpackMode.Completely,
        ...
        public void ShowNextTutorial()
        {
            ...
            if (SaveTutorialProgress)
                PlayerPrefs.SetInt("ns_savedTutorialIndex_" + SaveKey,
                    currentTutorialIndex - 1 <= 0 ? 0 : currentTutorialIndex - 1);
        }
        ...
        public bool StageStarted(int tutorialIndex, int stageIndex)
        {
            ...
            {
                DebugLog("Stage not found! tutorial index: " + tutorialIndex + ", stage index: " + stageIndex,
                    gameObject, DebugType.Error);
                return false;
            }

            if (currentTutorialIndex - 1 == tutorialIndex &&
                Tutorials[tutorialIndex].CurrentStageIndex - 1 == stageIndex)
            {
